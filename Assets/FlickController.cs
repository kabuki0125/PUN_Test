...
public class FlickController : ViewBase
{
    ...
    public event Action<float/*veloX*/,float/*veloY*/> DidDrag;
    /// <summary>ドラッグ時に一回だけ呼ばれるイベント.</summary>
    public event Action<float/*veloX*/,float/*veloY*/> DidDragOnce;
    /// <summary>ドラッグ開始時のイベント.</summary>
    public event Action DidTap;
        ...
        while(m_bTap){
            ...
            if(inputVelocity.magnitude > m_thresholdFlick){
                ...
                if(!m_bCallDragOnce){
                    ...
                    if(DidDragOnce != null){
                        DidDragOnce(inputVelocity.x, inputVelocity.y);
                    }
                }
                // ドラッグイベントコール.
                if(DidDrag != null){
                    DidDrag(inputVelocity.x, inputVelocity.y);
                }
            }
